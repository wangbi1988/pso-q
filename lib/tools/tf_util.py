# -*- coding: utf-8 -*-
"""
Created on Wed Nov  4 14:07:45 2020
...
class _Function(object):
    def __init__(self, inputs, outputs, updates, givens):
        ...
        for inpt in inputs:
            if not hasattr(inpt, 'make_feed_dict') and not (isinstance(inpt, tf.Tensor)and len(inpt.op.inputs) == 0):
                print("inputs should all be placeholders, constants, or have a make_feed_dict method")
                return None
        self.inputs = inputs
        updates = updates or []
    ...
    def __call__(self, *args, sess=None, **kwargs):
        """
        only run after all parameters are assigned.
        """
#        assert len(args) <= len(self.inputs), "Too many arguments provided"
        ...
        if len(self.inputs) == len(feed_dict):
